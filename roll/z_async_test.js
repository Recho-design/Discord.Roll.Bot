...
const gameName = function () {
  return "【Wiki查询/图片搜索】 .wiki .image .tran";
};
...
const getHelpMessage = async function () {
  return `【Wiki查询/即时翻译】.wiki .image .tran .tran.(目标语言)
Wiki功能		： .wiki (条目)  
EG: .wiki BATMAN  

图片搜寻功能	： .Image (内容)  
从Google 得到相关随机图片Link
随机YES NO: 如.image yesno 会得到yes 或NO 结果
...
常用语言代码: 英=en, 简=zh-cn, 德=de, 日=ja
语系代码 https://github.com/vitalets/google-translate-api/blob/master/languages.js

注: 翻译使用Google Translate
`;
};
  ...
  switch (true) {
	...
	case /^[.]translate$/i.test(mainMsg[0]): {
		...
		if (/^off$/i.test(mainMsg[1])) {
			...
			rply.text = '此频道已关闭翻译功能。'
			return rply
		}

		rply.text = '没有正确指令，需要输入.translate on 或.translate off 去启动/关闭翻译功能'
