...
const gameName = function () {
	return '【趣味掷骰】 排序(至少3个選項) choice/随机(至少2个選項) 运势 每日塔罗 每日笑话 每日动漫 每日一言 每日废话 每日黄历 每日毒汤 每日情话 每日灵签 每日浅草签 每日大事 每日(星座) 每日解答	立flag .me'
}
...
const prefixs = function () {
	return [{
		first: /^排序|排序$|^随机|随机$|^choice|^每日塔罗|^时间塔罗|^大十字塔罗|立flag|运势|鸭霸兽|^每日笑话$|^每日动漫$|^每日一言$|^每日废话$|^每日黄历$|^每日毒汤$|^每日情话$|^每日灵签$|^每日浅草签$|^每日大事$|^每日解答$|^每日白羊$|^每日牡羊$|^每日金牛$|^每日双子$|^每日巨蟹$|^每日狮子$|^每日处女$|^每日天秤$|^每日天平$|^每日天蠍$|^每日天蝎$|^每日射手$|^每日人马$|^每日摩羯$|^每日山羊$|^每日水瓶$|^每日宝瓶$|^每日双鱼$/i,
		second: null
	}]
}
...
const rollDiceCommand = async function ({
	...
	//choice 指令開始於此
	//	if (trigger.match(/choice|随机|選項|選1/) != null && mainMsg.length >= 3) return exports.funny.choice(inputStr, mainMsg);
	//tarot 指令
	/*
	...
	*/
	...
	//		if (trigger.match(/运势/) != null) return exports.funny.randomLuck(mainMsg); //占卜運氣		
	/*猜拳指令
...
*/
	...
	switch (true) {
		case /^help$/i.test(mainMsg[1]):
			...
			rply.buttonCreate = ['随机 跑團 溫習 打遊戲', '排序 A君 C君 F君 G君', '.re 簽到', '.re 1d100', '今日运势', '每日塔罗', '立FLAG', '每日大事', '每日笑话', '每日废话', '每日一言', '每日黄历', '每日毒汤', '每日情话', '每日灵签', '每日浅草签', '每日动漫', '每日解答']
			return rply;
		...
		//白羊座、金牛座、双子座、巨蟹座、狮子座、处女座、天秤座、天蠍座、射手座、摩羯座、水瓶座、双鱼
...
const twelveAstro = [
	'牡羊', '金牛', '双子', '巨蟹', '狮子', '处女', '天秤', '天蠍', '射手', '摩羯', '水瓶', '双鱼'
]
...
class TwelveAstro {
	...
	returnStr(astro, name) {
		return `今日${name}座運程
你的幸運数字：${astro.TODAY_LUCKY_NUMBER}	
...
「待一切結束後記得還給我。」', '\
「真希望這份幸福可以永遠持續下去。」', '\
「這工作結束後我們两人一起生活吧！」（この仕事が終わったら2人で暮らそう）', '\
「我們三个人要永永遠遠在一起！」', '\
「這是我女兒的照片，很可愛吧？」', '\
「請告訴他/她，我永遠愛他/她」', '\
...
async function axiosDaily(url) {
	...
	if (reply === '錯誤error') {
		reply = `伺服器出現問題，請稍後再試，如果問題持續数天，可以到支援群回報。`;
	}
	return reply;
...
const discordCommand = [
	{
		...
		async execute(interaction) {
			...
			if (text !== null) {
				await interaction.reply({ content: '已进行模擬HKTRPG說話', ephemeral: true }).catch();
				return `.me ${text}`
			}
			else return `需要输入內容\n 
	...
	},
	{
		data: new SlashCommandBuilder()
			...
			.addStringOption(option => option.setName('text').setDescription('输入所有內容，以空格分隔 如 排序 選項A 選項B 選項C').setRequired(true))
		,
		async execute(interaction) {
			const text = interaction.options.getString('text')
			if (text !== null)
	...
	},
	{
		data: new SlashCommandBuilder()
			.setName('随机')
			.setDescription('进行随机抽選')
			.addStringOption(option => option.setName('text').setDescription('输入所有內容，以空格分隔 如 選項A 選項B 選項C').setRequired(true))
		,
		async execute(interaction) {
			const text = interaction.options.getString('text')
			if (text !== null)
	...
	},
	{
		data: new SlashCommandBuilder()
			.setName('运势')
			.setDescription('进行随机抽選')
			.addStringOption(option => option.setName('text').setDescription('可選: 什麼的运势'))
		,
		async execute(interaction) {
...
class DailyFuckUp {
	...
	static discuss = [
		...
		"帶著這些問題，我們來審視一下主題。 ",
		"所謂主題，關鍵是主題需要如何寫。 ",
		"我們一般認为，抓住了問題的關鍵，其他一切則會迎刃而解。 ",
		"問題的關鍵究竟为何? ",
		"主題因何而發生?",
		"每个人都不得不面對這些問題。 在面對這種問題時， ",
		"一般來講，我們都必須務必慎重的考慮考慮。 ",
		"要想清楚，主題，到底是一種怎麼樣的存在。 ",
		"瞭解清楚主題到底是一種怎麼樣的存在，是解決一切問題的關鍵。 ",
		"就我个人來說，主題對我的意義，不能不說非常重大。 ",
		"本人也是經過了深思熟慮，在每个日日夜夜思考這个問題。 ",
		"主題，發生了會如何，不發生又會如何。 ",
		"在這種困難的抉擇下，本人思來想去，寢食難安。 ",
		"生活中，若主題出現了，我們就不得不考慮它出現了的事實。 ",
		"這種事實對本人來說意義重大，相信對這个世界也是有一定意義的。 ",
		"我們都知道，只要有意義，那麼就必須慎重考慮。 ",
		"在現今社會，一些重要的問題始終存在著。因此，我們需要關注這些問題並找到有效的解決方案。",
		"從長遠來看，我們必須重視某些問題的影響，因为它們可能對我們的未來產生深遠的影響。",
		"解決問題需要集中精力和全面的思考。只有這樣，才能找到最佳解決方案。",
		"我們必須從多个角度來看待問題，因为問題的解決通常不是單一的方法。",
		...
		"解決問題需要有一个清晰的目標和计劃。只有這樣，我們才能更有效地實現我們的目標。",
		"面對困難和挑戰，我們必須堅持不懈，直到找到最佳解決方案。",
		"在解決問題的過程中，我們必須有耐心和毅力。只有這樣，我們才能成功地克服所有的障礙。",
		"綜觀主題的歷史，我們會發現，這是一个複雜且多變的問題。",
		"許多學者和專家已經對主題进行了深入的研究和分析，但仍有許多問題需要解決。",
		...
		"解決主題需要全社會的參與和努力，不能單靠某一个群體或个人的力量。",
	...
	static celebrityQuotes = [
		...
		"約翰·藍儂曾經說過：“生命是發生在你身上的事情，當你忙於为其餘的東西而忘了它時，它就會溜走。這不禁令我深思",
		...
		"亞伯拉罕·林肯曾經說過：“你可以愛上你的工作，也可以恨你的工作，但你必須为它付出努力。這不禁令我深思",
		"比爾·蓋茨曾經說過：“成功不是取決於你有多聰明，而是取決於你有多認真。這不禁令我深思",
		"納爾遜·曼德拉曾經說過：“教育是改變世界的最強大的武器。這不禁令我深思",
		"史蒂夫·喬布斯曾經說過：“你的工作將佔用你生命中大部分時間，为什麼不要做你熱愛的工作呢？這不禁令我深思",
		...
		"奧普拉·溫弗瑞曾經說過，你相信什麼，你就成为什麼樣的人。這不禁令我深思",
		"呂凱特曾經說過，生命不可能有两次，但許多人連一次也不善於度過。這不禁令我深思",
		...
		"日本諺語曾經說過，不幸可能成为通向幸福的橋樑。這不禁令我深思",
		...
		"歌德曾經說過，決定一个人的一生，以及整个命運的，只是一瞬之間。這不禁令我深思",
		"卡耐基曾經說過，一个不注意小事情的人，永遠不會成就大事業。這不禁令我深思",
		...
		"富蘭克林曾經說過，你熱愛生命嗎？那麼別浪費時間，因为時間是組成生命的材料。這不禁令我深思",
		...
		"達爾文曾經說過，敢於浪費哪怕一个鐘頭時間的人，說明他還不懂得珍惜生命的全部價值。這不禁令我深思",
		"佚名曾經說過，感激每一个新的挑戰，因为它會鍛造你的意志和品格。這不禁令我深思",
		"奧斯特洛夫斯基曾經說過，共同的事業，共同的鬥爭，可以使人們產生忍受一切的力量。　這不禁令我深思",
		"蘇軾曾經說過，古之立大事者，不惟有超世之才，亦必有堅忍不拔之志。這不禁令我深思",
		"王陽明曾經說過，故立志者，为學之心也；为學者，立志之事也。這不禁令我深思",
		"歌德曾經說過，讀一本好書，就如同和一个高尚的人在交談。這不禁令我深思",
		"烏申斯基曾經說過，學習是勞動，是充滿思想的勞動。這不禁令我深思",
		"別林斯基曾經說過，好的書籍是最貴重的珍寶。這不禁令我深思",
		"富蘭克林曾經說過，讀書是易事，思索是難事，但两者缺一，便全無用處。這不禁令我深思",
		"魯巴金曾經說過，讀書是在別人思想的幫助下，建立起自己的思想。這不禁令我深思",
		"培根曾經說過，合理安排時間，就等於節約時間。這不禁令我深思",
		"屠格涅夫曾經說過，你想成为幸福的人嗎？但願你首先學會吃得起苦。這不禁令我深思",
		...
		"亞伯拉罕·林肯曾經說過，我這个人走得很慢，但是我從不後退。這不禁令我深思",
		"美華納曾經說過，勿問成功的秘訣为何，且盡全力做你應該做的事吧。這不禁令我深思",
		"俾斯麥曾經說過，對於不屈不撓的人來說，沒有失敗這回事。這不禁令我深思",
		"阿卜·日·法拉茲曾經說過，學問是異常珍貴的東西，從任何源泉吸收都不可恥。這不禁令我深思",
		"白哲特曾經說過，堅強的信念能贏得強者的心，並使他們變得更堅強。 這不禁令我深思",
		"查爾斯·史考伯曾經說過，一个人幾乎可以在任何他懷有無限熱忱的事情上成功。 這不禁令我深思",
		...
		"史美爾斯曾經說過，書籍把我們引入最美好的社會，使我們認識各个時代的偉大智者。這不禁令我深思",
		"馮學峰曾經說過，當一个人用工作去迎接光明，光明很快就會來照耀著他。這不禁令我深思",
		"吉格·金克拉曾經說過，如果你能做夢，你就能實現它。這不禁令我深思",
	]

	static afterFuck = ["這不禁令我深思。 ", "帶著這句話，我們還要更加慎重的審視這个問題： ", "這啓發了我， ", "我希望諸位也能好好地體會這句話。 ", "這句話語雖然很短，但令我浮想聯翩。 ", "無可否認，這句話帶給我們極大的啟示。", "我深深體會到這句話所蘊含的深意。", "這句話真正引起了我的共鳴。", "這句話不僅引發了我們的關注，也引起了我們的思考。", "我們需要認真對待這句話所提出的挑戰。", "這句話所傳達的信息絕對不容忽視。", "這句話令我們更加清晰地看到了問題的本質。", "這句話讓我們看到了問題的另一面。", "我深信這句話會成为我們思考的重要起點。", "我們必須從這句話中學到更多的東西。", "這句話能夠激發我們內心深處的共鳴。", "我們需要從這句話中學到一个重要的教訓。", "這句話引起了我們對問題的關注，也啟發了我們的思考。", "這句話不僅是一句警句，更是一个重要的提醒。", "這句話在我們思考的過程中發揮了重要的作用。", "這句話讓我們看到了一个全新的視角。", "這句話可以幫助我們更好地理解問題的本質。", "我們必須從這句話中吸取更多的智慧和啟示。", "這句話深刻地反映了現實的困境和挑戰。", "這句話讓我們更加明白了自己的不足之處。", "這句話揭示了問題的一个重要方面。", "這句話讓我們更加認識到自己的責任和使命。", "這句話提醒我們要時刻保持警醒和警覺。", "這句話讓我們更加堅定了自己的信念和決心。", "這句話可以幫助我們更好地理解自己和他人。", "這句話是一个重要的思想火花，可以引發更多的啟示。", "這句話可以幫助我們更好地理解自己的身份和使命。", "這句話讓我們更加明白了人生的真諦和意義。", "這句話可以激勵我們更加努力地工作和生活。", "這句話是一个非常寶貴的啟示和提醒。", "這句話讓我們看到了問題的一个新的方向和出路。", "這句話可以幫助我們更好地面對人生的挑戰和困境。", "這句話讓我們更加明白了自己的優點和不足。", "這句話是一个非常實用的工作和生活的指導原則。", "這句話可以幫助我們更好地理解人性和社會。", "這句話讓我們更加意識到自己的權利和義務。", "這句話讓我們更加了解了一个文化或一个國家的特點和價值觀。", "這句話可以啟發我們更多的創造力和想像力。", "這句話讓我們更加明白了生命的珍貴和脆弱。"]

	static formerFuck = ["曾經說過", "在不經意間這樣說過", "事先聲明", "先說一聲", "需要先強調", "需要先說明", "需要先說明一下", "必須說明的是", "講過一个小故事", "討論過這問題", "曾經稍微講過背景", "曾經簡單提過一下", "談到這个話題", "想要先聲明的是", "在關於這个問題", "根據自己的經驗", "曾探討過這个議題", "在談論過這件事", "過交代過", "談到這个事情時，說過", "在進入正題前，曾說過", "關於這个話題，曾說過", "交代過一下", "說過自己的立場", "闡述過想法", "探討過這个問題", "談論過這个主題", "曾分析過", "提過，一下問題的重要性", "曾深入探討這个問題", "談到這个議題"]
...
const dailyAnswer = ["不一定", "需要別人的幫助", "需要慎重考慮", "相信你自己", "你是對的", "放棄吧", "聽聽別人的建議", "需要堅持", "不要放棄", "不要錯過機會", "會有转機", "等待機會", "花更多時間來决定", "再多考慮", "你可能要放棄些東西", "考慮下別人的感受", "這事不靠譜", "別讓它影響到你", "做能讓你快樂的那个决定", "掃清障礙", "不要覺得憂慮", "主動一點", "時間會給你答案", "現在就開始", "別猶豫", "决定了就做", "顯而易見的結果", "保存實力", "時機還不成熟", "你需要掌握更多的信息", "去找个人傾訴", "你需要去探索真相", "把握機會", "决定了就堅持", "很麻煩2現在比以往任何時候的情况都要好", "重新思考", "列出原因", "期待一下,令人期待的事情馬上會發生", "培養一項新的愛好", "走容易走的路", "時間不對", "給自己點時間", "坦誠相告", "著眼未來", "信任", "別傻傻等待", "希望渺茫", "需要新的開始", "其實你已經有了答案", "聽聽別人的建議", "試著放棄", "不要猶豫", "趁早放棄", "再努力一些", "忘掉過去", "可以", "值得一試", "抓住機會", "不要嘗試", "聽長輩的建議", "不要堅持", "你可以的", "不靠譜", "打消念頭", "等待機會", "重新计劃", "重新開始", "擺脫現在的環境", "建議多次嘗試", "需要休息一下再决定", "冷靜思考再决定", "珍惜他或者她", "坦白一切", "努力一下", "主動出擊", "不要太主動", "冷靜處理", "謹慎做决定", "獨立面對", "從過去尋找答案", "多和家人溝通", "多和朋友溝通", "暗中觀察", "不太確定", "沒太大可能", "沒什麽把握", "學會放棄", "放弃這个念頭", "不值得一試", "風險很大", "不要再浪費時間", "做多重计劃", "再堅持一下", "不能繼續下去", "不會有結果", "結果不會讓你滿意", "結果出乎你的意料", "堅持就有結果", "付諸行動", "你會成功", "成功率很高", "沒問題", "耐心處理", "不要主動出擊", "好運馬上來了", "會有變化", "無濟於事", "是个好主意", "不太穩妥", "放空自己", "信任", "相信自己的判斷", "堅持就能看見真理", "會有转折", "會有改變", "相信自己的第一直覺", "定下目標", "學會獨立思考", "學會捨得", "繼續前行", "不懼未來", "需要些時間", "還有更好的選擇", "不合適", "結果不理想", "抓住新的機會", "尋找新的機會", "尋找更好的方法", "聽取家人的建議", "接受它", "當面溝通", "多次嘗試", "你一定會成功", "可以確定是的", "不重要", "錯誤的想法", "爭取機會", "或許很難", "放心去嘗試", "沒有好結果", "花點時間處理", "堅持自己的想法", "多方面思考再决定", "別猶豫", "思考風險再决定", "有希望", "不要失去信心", "擺脫現在的關係", "十分困難", "需要一些準備", "需要條件", "改變自己再决定", "參考朋友的建議", "分享想法會有收穫", "不算是", "考慮全面", "非常肯定", "也許希望很小", "不是最佳選擇", "再找找別的辦法", "趁早放棄", "一定要堅持", "時間會改變一切", "充實自己再做决定", "從回憶中找答案", "不可以嘗試", "不要做讓自己後悔的事", "不做你會後悔", "抓緊行動", "機不可失", "等待好機會", "整理思路", "可以確定", "控制自己", "做充分準備", "需要好的建議", "幷沒有那麽好", "不是最好的選擇", "不要抱太大希望", "完全正確", "很遺憾", "這不是一个好辦法", "不能否認", "千真萬確", "一定是", "完全肯定", "尋找可能", "細心觀察", "勇於面對", "爲未來做打算", "背向而馳", "憑藉自己的直覺", "深思熟慮再决定", "不是唯一的選擇", "最好的選擇", "找个人給你點意見", "請教你媽媽", "誰說的准呢先觀望著", "把心踹懷裏", "答案在鏡子裏", "這事兒不靠譜", "天上要掉餡餅了", "有好運", "要有耐心", "你需要知道真相", "還有另一種情况", "觀望", "別讓他影響到你", "照你想做的那樣去", "但行好事莫問前程", "走容易走的路", "試試賣萌", "借助他人的經驗", "再多考慮", "機會稍縱即逝", "制定一个新计劃", "GO", "情况很快會發生變化", "转移你的注意力", "告訴自己什麽是最重要的", "爲什麽不", "別傻等了", "不要忘記", "WHY", "NOT", "去解决", "尋找更多的選擇8上帝爲你關一扇門必定會爲你開一扇窗", "隨波逐流未必是好事", "問天問大地不如問問自己", "你就是答案", "去爭取機會", "改變不了世界就改變自己", "主動一點人生會大不相同", "學會妥協", "掌握更多信息", "相信你的最初想法", "勿忘初心方得始終", "掃清障礙", "把重心放在工作學習上", "培養一項新的愛好", "對他人慷慨", "去做其他的事情", "觀察形勢", "休息休息一會兒", "這是你最後的機會", "再考慮一下", "幷不明智", "等待更好的", "很快能解决", "重要", "去做", "不要過火", "事情開始變得有趣了", "保存你的實力", "不確定因素有點多", "結果不錯,你可能不得不放棄其他東西", "不要憂慮", "不需要", "去傾訴,告訴別人這對你意味著什麽", "無論你做何種選擇結果都是對的", "保持頭腦清醒", "克服困難", "實際一點", "你需要一點幫助", "協作", "尋找更多的選擇", "負責", "阻止", "你必須現在就行動", "遵守規則", "堅持", "需要花點時間", "你不會失望", "不要迫於壓力而改變初衷", "不要忽略身邊的人", "抗拒", "不值得鬥爭", "玩得開心就好", "毋庸置疑", "你也許會失望", "去改變", "一个强有力的承諾將會換回更好的結果", "也許有更好的解决方案", "不要害怕", "想法太對選擇太少", "一笑而過", "取决於你的選擇", "隨他去", "你需要考慮其他方面", "一年後就不那麽重要了", "醒醒吧別做夢了", "意義非凡", "默数十秒再問我", "去行動", "發揮你的想像力", "保持冷靜", "你必須彌補這个缺點", "你會後悔的", "毫無疑問", "當然", "現在比以往任何時候的情况都要好", "相信你的直覺", "這是一个機會", "去問你爸爸", "從來沒有", "尋找一个指路人", "去嘗試", "荒謬", "不賭", "不值得冒險", "不妥協", "關注你的家庭生活", "肯定", "不可預測", "絕對不", "我確定", "儘早完成,令人期待的事情馬上就要發生", "你需要適應", "表示懷疑", "它會帶來好運", "看看會發生什麽", "記錄下倆", "不宜在這个時候", "决定了就去做", "別要求太多", "放棄第一个方案", "Hold不住", "謹慎小心", "注意細節", "注意身後", "不要猶豫", "繼續前行", "情况很快會發生改變", "不要被情緒左右", "转移注意力", "著眼未來", "問自己什麽是最重要的", "不要等了", "保持樂觀", "沒有更好的選擇", "你需要主動", "妥協", "有比這更重要的東西", "你需要掌握更多的信息", "删除記憶", "專注於你的工作", "你需要考慮其他的方面", "相信自己的直覺", "形勢不明", "先讓自己休息", "重新考慮", "不要做的太過分", "保持現狀/有意料之外的事會發生不妨等待", "花更多的時間來决定", "你開心就好", "有風險但也有機會", "算了吧", "當然咯", "千萬別傻,保持你的好奇心去挖掘真相", "把心揣懷裏", "時機不對", "照你想做的那樣去做", "量力而行", "拋棄首選方案", "最佳方案不一定可行", "注意細節", "說出來吧", "誰都不能保證", "不要陷得太深", "至關重要", "這是一定的", "不妨賭一把", "需要多思考一下", "這个問題確實不好回答", "其實都還不錯", "你認爲好的那个", "或許還沒有", "沒有足够的條件", "目前不滿足", "可以接受", "停止", "對比一下再决定", "勿忘初心", "不重要", "多讀書少思考", "放棄第一个選擇", "不該堅持", "學會放棄", "捨得才有機會獲得", "你是對的", "你值得這麽做", "沒有你想的那麽簡單", "不會更糟糕", "別騙自己", "想太多了", "睡一覺再决定", "不是最佳選擇", "不合適", "把注意力转移一下", "不要强求", "時間會告訴你答案", "這件事不好回答", "要看你自己", "這个問題沒有答案", "你懂得，不用問我", "用心去做", "不能言傳", "改變自己", "無所謂", "全力以赴", "爭取早日擺脫", "顯而易見的道理", "沒有理由拒絕", "想想未來吧", "開心就好", "及時行樂", "看情况再說", "不聽老人言，吃虧在眼前", "無須多言", "熬過去就好", "一切都是好的", "是非難辨", "搞不清楚狀况", "不要太樂觀", "用心感受", "嗯", "明天就有變化", "等一周再說", "都可以", "都值得去做", "太早决定不好", "別懷疑自己", "你要果斷一些", "靜觀其變", "看起來不靠譜", "放輕鬆", "不想要就趁早放棄", "尋找新的開始", "都可以", "放下吧", "忽略別人的看法", "不需要解釋", "愛拼才會贏", "讓他、她知道", "其他選擇", "沒有意義", "你的答案在心裏", "換位思考", "嘗試新的生活", "接受它", "一切都是最好的安排", "完美", "不要放縱自己", "跟隨大衆的審美", "不太滿意的結果", "沒有更好的選擇", "堅持到底", "不要", "隨心所欲", "大膽去做", "聽人勸吃飽飯", "你還是不够努力", "不要欺騙自己", "注意細節", "珍惜現在", "讓別人替你分擔", "分享會有驚喜", "走下去", "淘汰它", "心誠則靈", "行與不行一試便知", "真心對待", "最後的决定", "二選一，選前者", "找人幫你做", "相信大家的眼光", "難得糊塗", "從現在開始努力", "回頭是岸", "求同存異", "或許還不是時候", "先苦後甜", "樹立信心再來一次", "過了這村沒這店", "運氣不佳不建議做", "別一條路走到黑", "別再委屈自己", "多看看外面的世界", "問下你們老師", "這是个問題嘛？", "無法回答", "相信科學", "少吃多動就會有收穫", "幹嘛想不開來笑一个", "誰也幫不了你", "瞭解自己的人會給你答案", "沒效果", "言多必失", "敞開心扉", "梳理一下再决定", "想想得了", "最後的疼愛是手放開", "別想那麽多沒用的", "沒用的", "不起作用", "適得其反", "空說無用", "沒什麽不妥", "長點心吧", "還有別的選擇嘛？", "別往心裏去", "控制自己", "今生無緣", "幸福快來了", "不是現在這个人、事、物", "再給自己一次機會", "未必適合你", "沒問題的", "不计得失方能成功", "愛幹嘛就幹嘛", "分散注意力", "緩解壓力繼續前行", "說多無益", "別膽少", "直接點", "只有你最清楚", "問問你閨蜜或基友", "看樣子是不行", "沒什麽差別", "摸著自己的胸再問一次", "親愛的那是不可能的", "反正也不會實現", "無所謂了", "試一次就知道", "別怕麻煩", "自己拿主意吧", "別人說的話隨便聽一聽", "我也幫不上忙", "和昨天一樣", "別忘了你的承諾", "恐怕來不及", "反復無常", "不要自討苦吃", "不要自討沒趣", "枉然", "取長補短", "不能硬來", "不明智的選擇", "犯不著", "理清頭緒答案就有了", "放輕鬆再問一遍", "你喜歡的就是好的", "如果有選擇我選第一个", "做自己喜歡的事", "很重要的事情要花點功夫", "對自己好一點", "愛惜自己", "沒有對比就沒有傷害", "醒醒吧", "不要輕易放棄", "浪費功夫", "依賴別人也不是辦法", "別人幫不了你", "沒有辦法感同身受", "不要好了傷疤忘了疼", "要矜持點", "簡單易行的方式", "找值得信賴的人諮詢", "少點套路", "什麽都沒有把握", "主意不錯哦", "要有野心", "好景不長", "不要自尋煩惱", "清理自己的過去", "提高自己", "誰也做不了你的主", "這个還真不好說", "給自己一點壓力", "別管對錯去做吧", "你需要點套路", "懶得想不如簡單點", "看開一點", "支持你", "不適合你的", "你這麽好看說什麽都對", "多讀書少提問", "活在當下", "別灰心再試一下", "沒有絕對的答案", "不存在優勢", "抓住重點", "這跟我沒關係", "好主意", "搞不定", "想想就好，別衝動", "鼓勵一下，你行滴", "無疑是一个好選擇", "看情况咯", "費盡心思也無濟於事", "性格不合", "試試賣萌、耍酷", "冷靜冷靜", "主動聯繫", "一包辣條壓壓驚", "痛苦的選擇", "離開", "顧及別人感受", "傻人有傻福", "一切從簡", "重新考慮一下", "千萬小心", "太天真", "別想太多啦", "忍一忍就過去了", "何必認真", "都是緣分", "提醒自己過去的事", "隨你吧", "這不重要吧", "你說對了呢", "仁者見仁智者見智", "無解", "是个謎", "無所謂", "不要反復果斷點", "不要感情用事", "放手一搏", "什麽都不用做", "转機馬上到了", "要敢於直面現實", "改變不了自己，就放弃", "接受現狀", "可能不會有", "現實很殘酷", "不知道啊", "你一定是對的", "跟以前一樣", "還是老樣子", "不如讓自己開心一點", "糟糕", "猜不透就不猜", "別理睬", "忍", "陽光總在風雨後", "小心爲上", "不提也罷", "不該問我，問問自己", "想不通就明天再想", "問你身邊的异性", "問你身邊的朋友", "問你身邊的同性", "答案即將揭曉", "肯定沒戲", "別抱太大希望", "慢慢來", "不必在乎", "沒有準確答案", "如往常一樣", "沒什麽不妥", "安心去做", "抓緊實現", "你搞不定", "這个問題沒有答案", "需要找个專家問問", "樂觀面對", "不要做鴕鳥", "清醒地認識自己", "擺脫一切幹擾", "試試手氣重新來過", "別讓自己變得不像自己", "別著急，再好好想想", "問天問地不如問問自己", "毫無意義的事", "不要强加於人", "及時行樂", "與人溝通，會有收穫", "樂趣在於探索", "找不到相關的信息", "大膽提出建議", "無話可說", "別忘了自己的夢想", "說好的獨立解决呢", "拒絕回答一切問題", "不太想管你這種閑事", "安心的去做", "難道告訴你結果不妙嘛", "無聊的問題", "別人說的都對", "好人有好報", "祈禱一下，就會有奇迹", "不够虔誠，重新問一次", "不要騙自己", "很尷尬的局面", "沒必要堅持", "放手一搏", "換个角度思考", "神仙都幫不了你", "心靈鶏湯救不了你", "遠水救不了近火", "更多選擇更多歡笑", "軟硬兼施", "全面推進", "妥協吧", "只是時間問題罷了", "天時地利只欠人和", "等風來", "回家問你媽媽", "不一定是你滿意的結果", "强扭的瓜不甜", "真的未必能做到", "沒可能完成", "嘗試三次不行就撤", "誰說你不行，去他打", "你怎樣做都是錯,真理永遠掌握在少数人手中", "別猶豫加油做", "去吧，不然會後悔", "智者是不需要任何答案的", "反向思考", "淡定", "不知道", "找个人請教一下", "話聽三分", "你的地盤你做主", "這个問題太深奧", "决定了就去做"]
