/**
 * @file candleDays.js
 * @description 检查今天是否是指定日期，如果是的话，回傳指定的字串，否则回傳空字串，用于检查是否需要显示蜡燭，或是其他用途。
 * @example 在.env 中设定 CANDLE_DATES=2,14,🌷 12,25,🕯️
 * @example 在程式中使用：
 *          const candle = require('../modules/candleDays.js'); 
 *          candle.checker();
 * @example 以上设定会在每年的 6 月 4 日显示蜡燭，2 月 14 日显示🌷，12 月 25 日显示🕯️
 * @example 日期格式为：月,日,显示的字串，月和日必须为数字，显示的字串可以不填，预设为🕯️
 * @example 日期之间以空白隔开，可以设定多个日期，例如：CANDLE_DATES=2,14,🌷 12,25,🕯️
 * 
 */
...
class CandleChecker {
    ...
    #scheduleFunction() {
        ...
        setTimeout(() => {
            this.#scheduleFunction(); // 设定下一次定时任务
            this.#updateToday(); // 更新今天的日期
            this.#checkForCandle();// 检查是否是指定日期，如果是的话，设定 this.isCandleDay 为 true
        }, msUntilMidnight); // 设定定时器等待到明天 00:00+5秒 后執行
    }
    #updateToday() {
