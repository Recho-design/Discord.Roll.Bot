let MoveList;
if (!MoveList) MoveList = [];
module.exports.MoveList = MoveList;

Array.prototype.push.apply(MoveList, [
  {
    name: "极巨虫蛊",
    alias: "ダイワーム|Max Flutterby",
    power: " 2",
    category: "hybrid",
    type: "Bug",
    tags: ["target|l|foe", "frame|target|特殊|down|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|sleep||number|d3",
      "frame|paralysis||number|d3",
      "frame|poison||number|d3",
      "pdice|l|2",
      "text|l|虫系招式|Bug",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "降低目标的特殊。",
    desc: "They say that the flutter of a butterfly can cause a hurricane in another part of the world. With this Max Move it can cause it right where it stands.",
  },
  {
    name: "极巨恶霸",
    alias: "ダイアーク|Max Darkness",
    power: " 2",
    category: "hybrid",
    type: "Dark",
    tags: ["target|l|foe", "frame|target|特防|down|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|sleep||number|d3",
      "frame|flinch||number|d3",
      "pdice|l|2",
      "effect|l|lethal",
      "text|l|恶系招式|Dark",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "降低目标的特防。",
    desc: "With its giganic size, the Pokémon blocks all sunlight around as if night suddenly fell upon the arena. Even after light returns you feel part of you was left in the abyss.",
  },
  {
    name: "极巨龙骑",
    alias: "ダイドラグーン|Max Wyrmwind",
    power: " 2",
    category: "hybrid",
    type: "Dragon",
    tags: ["target|l|foe", "frame|target|力量|down|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "effect|l|lethal",
      "frame|flinch||number|d3",
      "frame|self|特质|up|1",
      "frame|target|意志点|down|1",
      "text|l|龙系招式|Dragon",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "降低目标的力量。",
    desc: "With a deafening roar and an explosive blast of pure rage this giant pokémon is hungry for destruction. No rest, no mercy, no matter what.",
  },
  {
    name: "极巨闪电",
    alias: "ダイサンダー|Max Lightning",
    power: " 2",
    category: "hybrid",
    type: "Electric",
    tags: ["target|l|foe", "text|l|电气场地|Electric"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|paralysis||always",
      "frame||异常状态|always",
      "effect|l|block",
      "frame|self|伤害|plus|d1",
      "text|l|电系招式|Electric",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动电气场地的效果。持续 4 个战斗轮。",
    desc: "An extremely dangerous lightning storm flashes through the arena, a single use of this Max Move could energize a city for weeks if not months at a time.",
  },
  {
    name: "极巨妖精",
    alias: "ダイフェアリー|Max Starfall",
    power: " 2",
    category: "hybrid",
    type: "Fairy",
    tags: ["target|l|foe", "text|l|薄雾场地|Fairy"],
    "gmax-tags": [
      "target|l|allfoe",
      "target|l|allally",
      "frame|heal||heal|2",
      "frame|confuse||always",
      "frame|love||always",
      "text|l|妖精招式|Fairy",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动薄雾场地的效果。持续 4 个战斗轮。",
    desc: "A shooting star in the night sky will make a wish come true. A thousand shooting stars in the battlefield must be a lot more efficient, right?",
  },
  {
    name: "极巨拳斗",
    alias: "ダイナックル|Max Knuckle",
    power: " 2",
    category: "hybrid",
    type: "Fight",
    tags: ["target|l|foe", "target|l|allally", "frame|self|力量|up|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "effect|l|crit",
      "frame|self|伤害|plus|d2",
      "frame|self|特质|down|never",
      "frame|flinch||number|d3",
      "text|l|格斗招式|Fight",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "提升使用者和范围内所有队友的力量。",
    desc: "Either through sheer muscle power or through focusing its inner chi, this giant Pokémon can pack a punch that will leave its poor victim splat into the ground.",
  },
  {
    name: "极巨火爆",
    alias: "ダイバーン|Max Flare",
    power: " 2",
    category: "hybrid",
    type: "Fire",
    tags: ["target|l|foe", "weather|l|sun"],
    "gmax-tags": [
      "target|l|allfoe",
      "weather|l|sun2",
      "text|l|无视特性",
      "frame|burn3||number|d3",
      "effect|l|block",
      "text|l|火系招式|Fire",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动大晴天天气状态的效果。持续 4 个战斗轮。",
    desc: "Even at night, the fiery flames comming out of this Pokémon light up the sky. This Max Move is an onslaught of heat that is definitely not good for the environment.",
  },
  {
    name: "极巨飞冲",
    alias: "ダイジェット|Max Airstream",
    power: " 2",
    category: "hybrid",
    type: "Flying",
    tags: ["target|l|foe", "target|l|allally", "frame|self|灵巧|up|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "weather|l|wind",
      "effect|l|crit",
      "frame|self|特质|down|never",
      "frame|heal|治疗状态|always",
      "text|l|飞行招式|Flying",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "提升使用者和范围内所有队友的灵巧。",
    desc: "Wind has two extremes, you are either with it or against it. It will be benevolent with its allies and ruthless with its enemies, blowing away any resistance.",
  },
  {
    name: "极巨幽魂",
    alias: "ダイホロウ|Max Phantasm",
    power: " 2",
    category: "hybrid",
    type: "Ghost",
    tags: ["target|l|foe", "frame|target|防御|down|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "effect|l|lethal",
      "frame|flinch||number|d3",
      "effect|l|block",
      "frame|target|意志点|down|1",
      "text|l|幽灵招式|Ghost",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "降低目标的防御。",
    desc: "The energy that creates nightmares floods the arena like a shadow, by the time this Max Move is casted is already too late, be prepared to suffer night terrors for weeks... if you survive.",
  },
  {
    name: "极巨草原",
    alias: "ダイソウゲン|Max Overgrowth",
    power: " 2",
    category: "hybrid",
    type: "Grass",
    tags: ["target|l|foe", "text|l|青草场地|Grass"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|target|灵巧|down|1",
      "text|l|无视特性",
      "frame|heal|治疗状态|always",
      "frame|target|闪避|down|1",
      "text|l|草系招式|Grass",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动青草场地的效果。持续 4 个战斗轮。",
    desc: "Nature becomes an entity of its own under this enormous Pokémon command, there is no stopping this burst of life coming violently from the ground.",
  },
  {
    name: "极巨大地",
    alias: "ダイアース|Max Quake",
    power: " 2",
    category: "hybrid",
    type: "Ground",
    tags: ["target|l|foe", "target|l|allally", "frame|self|特防|up|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|self|伤害|plus|d2",
      "effect|l|block",
      "effect|l|crit",
      "text|l|无视特性",
      "text|l|地面招式|Ground",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "提升使用者和范围内所有队友的特防。",
    desc: "A trerrible quake shakes the earth to its very core, there are reports of nearby cities reporting aftershocks after this Max Move is used in the arena.",
  },
  {
    name: "极巨寒冰",
    alias: "ダイアイス|Max Hailstorm",
    power: " 2",
    category: "hybrid",
    type: "Ice",
    tags: ["target|l|foe", "weather|l|hail"],
    "gmax-tags": [
      "target|l|allfoe",
      "effect|l|lethal",
      "frame|frozen||number|d3",
      "pdice|l|2",
      "effect|l|block",
      "text|l|冰系招式|Ice",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动冰雹天气状态的效果。持续 4 个战斗轮。",
    desc: "A harsh blizzard is unleashed and the temperature drops below zero in the whole city. Revenge is a dish best served with this Max Move.",
  },
  {
    name: "极巨攻击",
    alias: "ダイアタック|Max Strike",
    power: " 2",
    category: "hybrid",
    type: "Normal",
    tags: ["target|l|foe", "frame|target|灵巧|down|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "effect|l|lethal",
      "frame|paralysis||number|d3",
      "frame|self|特质|up|1",
      "frame|self|伤害|plus|d2",
      "text|l|一般招式|Normal",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "降低目标的灵巧。",
    desc: "Normal types are often disregarded as weak, unimpressive, plain, and over all second-rate. Normal types have had enough of everyone’s Tauros’ waste.",
  },
  {
    name: "极巨酸毒",
    alias: "ダイアシッド|Max Ooze",
    power: " 2",
    category: "hybrid",
    type: "Poison",
    tags: ["target|l|foe", "target|l|allally", "frame|self|特殊|up|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|poison2||number|d3",
      "frame|burn1||number|d3",
      "frame|target|特质|down|1",
      "effect|l|lethal",
      "text|l|毒系招式|Poison",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "提升使用者和范围内所有队友的特殊。",
    desc: "A virulent and corrosive ooze floods the arena, the stench is so pungent it makes everyone sick. Whole towns have had to be evacuated for cleaning tasks after this Max Move was used.",
  },
  {
    name: "极巨超能",
    alias: "イサイコ|Max Mindstorm",
    power: " 2",
    category: "hybrid",
    type: "Psychic",
    tags: ["target|l|foe", "text|l|精神场地|Psychic"],
    "gmax-tags": [
      "target|l|allfoe",
      "text|l|无视特性",
      "frame|sleep||number|d3",
      "frame|confuse||always",
      "effect|l|neverfail",
      "text|l|超能招式|Psychic",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动精神场地的效果。持续 4 个战斗轮。",
    desc: "This Max Move targets the only place the foe cannot escape, through a psychic assault it can melt the mind of its enemies, leaving them in a catatonic state for weeks.",
  },
  {
    name: "极巨岩石",
    alias: "ダイロック|Max Rockfall",
    power: " 2",
    category: "hybrid",
    type: "Rock",
    tags: ["target|l|foe", "weather|l|sand"],
    "gmax-tags": [
      "target|l|allfoe",
      "effect|l|lethal",
      "frame|flinch||number|d3",
      "frame|self|伤害|plus|d2",
      "frame|target|特质|down|1",
      "text|l|岩石招式|Rock",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动沙暴天气状态的效果。持续 4 个战斗轮。",
    desc: "A tsunami of rubble falls towards the target. With no escape in sight, it is trapped between a rock and a hard place and more rocks. Oh! so many more rocks.",
  },
  {
    name: "极巨钢铁",
    alias: "ダイスチル|Max Steelspike",
    power: " 2",
    category: "hybrid",
    type: "Steel",
    tags: ["target|l|foe", "target|l|allally", "frame|self|防御|up|1"],
    "gmax-tags": [
      "target|l|allfoe",
      "frame|self|特质|down|never",
      "frame|flinch||number|d3",
      "frame|self|受到伤害|minus|2",
      "frame|self|伤害|plus|d2",
      "text|l|钢系招式|Steel",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "提升使用者和范围内所有队友的防御。",
    desc: "The Pokémon calls upon large sharp pikes of steel that pierce everything on their way. Although incredibly destructive, the steel plates left in the field provide a sturdy cover. ",
  },
  {
    name: "极巨水流",
    alias: "ダイストリーム|Max Geyser",
    power: " 2",
    category: "hybrid",
    type: "Water",
    tags: ["target|l|foe", "weather|l|rain"],
    "gmax-tags": [
      "target|l|allfoe",
      "weather|l|rain2",
      "text|l|无视特性",
      "frame|target|特质|down|1",
      "frame|burn1||number|d3",
      "text|l|水系招式|Water",
    ],
    accuracy: "原招式命中   2",
    damage: "力量/特殊   原招式伤害   2",
    effect: "发动下雨天气状态的效果。持续 4 个战斗轮。",
    desc: "Boiling water spouts from a geyser right below the target, the water is shot so high up it comes down as rain for the next few days.",
  },
]);
